...
| Bedrock auth pattern | ✅ PASS | D3 reuses the existing [src/llm_client.py](../../../src/llm_client.py) Bedrock client and does not bypass ABSK-backed config |
...
- grounded the mocked D3 end-to-end test in the real `pason-well-service` QA trigger example
